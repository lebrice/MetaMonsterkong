...
class MonsterKong_Base(PyGameWrapper):
    ...
    def __init__(self, mk_config):
        ...
        self.height = int(self.mk_config['MapHeightInTiles']) * 15 #modify height accordingly based on how long the game level is , for map_short it was 210*210, for short2 it was 270*270, for short3 it is 170*170
        self.width = int(self.mk_config['MapWidthInTiles']) * 15
...
class MonsterKong_RandomEnsemble(MonsterKong_Base):
    def __init__(self, mk_config):
        super(MonsterKong_RandomEnsemble, self).__init__(mk_config=mk_config)

    def init(self):
        # Create a new instance of the Board class
        self.texture_fixed = self.mk_config['TextureFixed']
        level = np.random.randint(low=int(self.mk_config['StartLevel']), high=int(self.mk_config['StartLevel'])+int(self.mk_config['NumLevels']))
