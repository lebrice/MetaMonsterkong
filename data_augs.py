...
class Grayscale(object):
    ...
    def do_augmentation(self, x):
        ...
        for i in range(self.num_frame_stack):
            x_copy_lst.append(x[:,3*i:3*(i+1),:,:].clone())
            x_copy_lst[-1] = self.transform(x_copy_lst[-1])
            x_copy_lst[-1] = x_copy_lst[-1].repeat([1,3,1,1])
        x_copy = torch.cat(x_copy_lst, dim=1)
        return x_copy
...
class CutoutColor(object):
    ...
    def __init__(self, 
        ...
        self.w1 = np.random.randint(self.box_min, self.box_max, batch_size)
        self.h1 = np.random.randint(self.box_min, self.box_max, batch_size)
        self.rand_box = np.random.randint(0, 255, size=(batch_size, 1, 1, 3*num_frame_stack), dtype=obs_dtype) / 255.
    ...
    def change_randomization_params(self, index_):
        self.w1[index_] = np.random.randint(self.box_min, self.box_max)
        self.h1[index_] = np.random.randint(self.box_min, self.box_max)
        self.rand_box[index_] = np.random.randint(0, 255, size=(1, 1, 1, 3*self.num_frame_stack), dtype=self.obs_dtype)

    def change_randomization_params_all(self):
        self.w1 = np.random.randint(self.box_min, self.box_max, self.batch_size)
        self.h1 = np.random.randint(self.box_min, self.box_max, self.batch_size)
        self.rand_box = np.random.randint(0, 255, size=(self.batch_size, 1, 1, 3*self.num_frame_stack), dtype=self.obs_dtype)
...
class RandomConv(object):
    ...
    def do_augmentation(self, x):
        ...
        for trans_index in range(num_trans):
            ...
            for i in range(self.num_frame_stack):                    
                temp_x = x[trans_index*batch_size:(trans_index+1)*batch_size, 3*i:3*(i+1)]
                temp_x = temp_x.reshape(-1, 3, img_h, img_w) # (batch x stack, channel, h, w)
                rand_out_lst.append(rand_conv(temp_x))
            
            rand_out = torch.cat(rand_out_lst, dim=1) 
...
class ColorJitter(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if random_inds.sum() > 0:
            new_inputs_lst = []
            for i in range(self.num_frame_stack):
                new_inputs_lst.append(self.transform(inputs[inds, 3*i:3*(i+1)]))            
            inputs[inds] = torch.cat(new_inputs_lst, dim=1)
        return inputs
